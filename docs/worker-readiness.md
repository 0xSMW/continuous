...
| Dispatch/Ops | partial | partial | partial | partial | partial | partial | partial | partial | partial | partial | partial | `schedule.propose`, `customer_update.draft`, and `closeout.prepare` slices are registered and CI-covered; exception routing and live calendar/customer-send credentials remain blocked | Extend Dispatch into exception routing, then prove scoped live calendar/customer-send execution gates before launch |

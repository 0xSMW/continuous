...
| Dispatch/Ops | partial | partial | partial | partial | partial | partial | partial | partial | partial | partial | partial | `schedule.propose`, `customer_update.draft`, `closeout.prepare`, and `exception.route` are registered and CI-covered; live calendar/customer-send credentials remain blocked | Prove scoped live calendar/customer-send execution gates before launch |
| Finance | partial | partial | partial | partial | partial | partial | partial | partial | partial | partial | partial | `invoice.prepare` is registered, CI/deploy-smoked, and produces cash packets from Dispatch closeouts; AR follow-up, cash forecast, payment draft, live accounting credentials, and dual-control execution gates remain blocked | Implement AR follow-up, cash forecast, payment draft, and live-accounting/payment readiness checks without adding finance-specific routes |

...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Revenue Operations | live | live | live | live | live | live | live | partial | live | partial | partial | `docs/revenue-operations-worker-v1-contract.md`; `src/worker/revenue.ts`; `src/worker/revenue.integration.test.ts`; `app/worker/route.test.ts`; deploy smoke in `.github/workflows/deploy.yml` for `lead.read`, `lead.classify`, `response.draft`, `quote.prepare`, `run`, approval continuation, adapter retry/reconcile, blocked external execution, and controlled-send receipt recording; `/worker` `view: "readiness"` reports dry-run proof checks and live credential gates | Production connector credentials and live provider egress remain blocked; scheduler polling needs real connection coverage | Provision production inbox/CRM and sender managed credential refs, create pollable active connections through `/core connection.upsert`, record readiness through `/core connection.health.record`, then run a production controlled-send receipt with rollback proof |

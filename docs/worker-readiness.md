...
| Offer and Pricing | planned | planned | planned | planned | planned | planned | planned | blocked | planned | planned | blocked | `docs/offer-pricing-worker-v1-contract.md`; `docs/worker-expansion.md`; `docs/worker-handoffs.md`; `src/worker/planned-workers.ts` planned `offer_pricing_operations` contract metadata | Missing first runtime handler, margin-policy fixture, discount approval packet, generated `price_policy` view, eval fixture, and deploy smoke | Prove the `revenue.quote_to_pricing` handoff fixture, then promote `margin.review.prepare` and `price_policy` without adding pricing-specific routes |
...
| `offer_pricing_operations.margin.review.prepare` | Planned contract; external price publish and customer sends are blocked | Revenue quote evidence, margin rule, discount policy, approval packet, generated `price_policy` view, and no-send proof | `revenue.quote_to_pricing` fixture with margin verdict, discount approval request, quote-line policy refs, and idempotent replay |

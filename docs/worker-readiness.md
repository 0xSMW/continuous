...
| Finance | partial | partial | partial | partial | partial | partial | partial | partial | partial | partial | partial | `invoice.prepare`, `ar_followup.draft`, `cash_forecast.generate`, and `payment_draft.prepare` are registered and produce cash packets from Dispatch closeouts, invoice evidence, account/cash-driver inputs, and bill/payment selectors; live accounting/payment credentials, bank adapter readiness, and dual-control execution gates remain blocked | Implement live-accounting/payment readiness checks and dual-control execution gates without adding finance-specific routes |

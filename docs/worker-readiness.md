...
| Finance | partial | partial | partial | partial | partial | partial | partial | partial | partial | partial | partial | `invoice.prepare`, `ar_followup.draft`, and `cash_forecast.generate` are registered and produce cash packets from Dispatch closeouts, invoice evidence, and account/cash-driver inputs; payment draft, live accounting credentials, and dual-control execution gates remain blocked | Implement payment draft and live-accounting/payment readiness checks without adding finance-specific routes |

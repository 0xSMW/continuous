...
| Finance | partial | partial | partial | partial | partial | partial | partial | partial | partial | partial | partial | `invoice.prepare` and `ar_followup.draft` are registered and produce cash packets from Dispatch closeouts and invoice evidence; cash forecast, payment draft, live accounting credentials, and dual-control execution gates remain blocked | Implement cash forecast, payment draft, and live-accounting/payment readiness checks without adding finance-specific routes |

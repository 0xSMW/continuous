...
| Dispatch/Ops | partial | partial | partial | partial | partial | partial | partial | partial | partial | partial | partial | `schedule.propose` and `customer_update.draft` slices are registered and CI-covered; closeout, exception routing, and live calendar/customer-send credentials remain blocked | Extend Dispatch into closeout packets and exception routing, then prove scoped live calendar/customer-send execution gates before launch |

...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Revenue Operations | live | live | live | live | live | live | live | partial | live | partial | partial | `docs/revenue-operations-worker-v1-contract.md`; `src/worker/revenue.ts`; `src/worker/revenue.integration.test.ts`; `app/worker/route.test.ts`; deploy smoke in `.github/workflows/deploy.yml` for `lead.read`, `lead.classify`, `response.draft`, `run`, approval continuation, adapter retry/reconcile, and blocked external execution | Production connector credentials and approved external send remain blocked; scheduler polling needs real connection coverage | Provision production connector credentials, create pollable active connections through `/core connection.upsert`, record readiness through `/core connection.health.record`, then prove scoped adapter execution with receipt and rollback proof |
...
| Workforce | planned | planned | partial | partial | partial | partial | partial | planned | planned | planned | planned | `docs/workforce-operations-worker-v1-contract.md`; `src/worker/planned-workers.ts`; seeded workflow/object/capability records in `src/db/seed.ts`; contract coverage in `src/worker/worker-contracts.test.ts` | Runtime is schema/seed backed only | Implement document packet fixture, payroll readiness workflow, and restricted-document proof |

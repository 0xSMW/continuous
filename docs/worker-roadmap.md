...
worker must reuse Continuous Core primitives before it gets a new runtime slice:
business graph, workflow runs and steps, approvals, capabilities, budget,
adapters, generated UI, evidence, and evals.

The first launch gate is the Core/platform foundation. Role workers can be
seeded and tested before that gate is complete, but no worker launch counts
until `/core`, `/worker`, `/workflow`, `/approval`, and `/app-server` share the
same tenant, capability, budget, evidence, approval, receipt, and eval rules.
...
| Approval | Shared `/approval` envelope supports the worker's approval subject |
| App-server control bridge | Shared `/app-server` controls call `/worker`, `/workflow`, and `/approval` envelopes without private worker routes |
...
Core/platform foundation is the first launch gate. It must ship before any role
worker is considered launched, even when runtime slices already exist.
...
## Phase 2: Revenue Operations Worker
...
Revenue is the first role-worker proof after the Core/platform launch gate. It
only counts when these gates are all live in the same deploy. The Core-approved
`customer_message.send` receipt-recording path now exists under
...
|---|---|
| Source coverage | Production `lead.read` runs from at least one live inbox or CRM connection created through `/core` with command `connection.upsert`, with `/core` command `connection.health.record` proof and scheduler cursor evidence |
...
## Phase 3: Owner Chief-of-Staff Worker
...
Owner orchestrates packaged-worker expansion after Systems: it assembles
cross-family commands, checks Core refs and approvals, opens the decision queue,
and keeps package execution on the generic API spine.
...
waves that compose the foundation and first worker set instead of creating
private APIs. Owner orchestrates each package: it selects the package, validates
Core refs, sequences worker commands, prepares the decision queue, and holds
execution behind approval until receipts and rollback evidence are present.

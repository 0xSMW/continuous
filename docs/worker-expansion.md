...
| Workforce | Person, Employment, ContractorEngagement, Position, CompensationAgreement, Credential, Document | `worker.read`, `document_packet.prepare`, `approval.request`, `payroll_preview.prepare` | Hire employee, engage contractor, credential renewal, schedule readiness, payroll input readiness | Docs/signature, calendar, HRIS/payroll, email | New-hire or contractor packet with documents, signatures, classification rationale, payroll blockers | Document completeness, classification review pass rate, payroll blocker detection |

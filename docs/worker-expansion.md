...
| 11 | Growth Worker | Planned `POST /worker` command `campaign.draft`, `view: "campaigns"` | Campaign, channel, audience, content draft, attribution event, budget reservation | `growth.campaign_to_owner_review` | Claims have source refs, budget is reserved, audience/channel are explicit | External publish approval and ROI ledger fixture |
...
|---|---|---|---|---|---|---|
| Revenue | Lead, Customer, Offer, Quote, Booking, Job, Invoice, Payment, Review | `lead.read`, `lead.classify`, `response.draft`, `quote.prepare`, `payment_link.prepare`, `schedule.propose`, `invoice.prepare`, `owner_brief.generate` | Lead intake, quote approval, payment-link packet prep, schedule proposal, invoice prep, collections follow-up, review request | Website forms, email, calendar, CRM/spreadsheet, accounting, payments | Lead-to-cash packet with source message, classification, quote draft, payment-link draft, approval, adapter receipt, object versions | Classification accuracy, quote policy adherence, approval rate, response latency, budget per workflow, no unauthorized provider link creation |

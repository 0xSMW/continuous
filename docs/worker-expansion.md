...
| Growth and Brand | Growth Worker | First runtime slice | Campaign, channel, audience, content draft, attribution event, review source | `growth_operations.campaign.draft` runs on `/worker` with source refs and policy under `config`; publish/send/spend/tracking mutation remains blocked |

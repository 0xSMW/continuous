...
| Growth and Brand | Growth Worker | First runtime slice | Campaign, channel, audience, content draft, attribution event, review source | `command=campaign.draft` runs on `/worker` with `worker.role=growth_operations` and source refs/policy under `config`; publish/send/spend/tracking mutation remains blocked |
...
| Compliance | RulePack, Obligation, FilingRequirement, FilingDraft, Notice, License, Permit, InsurancePolicy | `filing.prepare`, `approval.decide`, `snapshot`, `obligations`, `packet` | Core obligation intake, notice response, license renewal, filing draft, evidence export | Document stores, calendar, agency portals/manual upload, email | Compliance packet with source refs, rule snapshot, draft, approval, receipt/rejection | Deadline coverage, rule-source traceability, false-positive blocker rate |

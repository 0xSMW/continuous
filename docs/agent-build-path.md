...
| `POST /api/core` | Canonical Core command surface with `command`, `core`, `config`, and `idempotencyKey` payload fields for tasks, task transitions, approvals, objects, object links, events, evidence, documents, decisions, and generated views |

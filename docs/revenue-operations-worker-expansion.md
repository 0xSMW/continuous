...
| Continuation command | `POST /worker` with payload `command: "continue"`, `idempotencyKey`, and `config.approvalId`; the continuation starts and completes through the Core worker-run lifecycle, approved decisions default to blocked no-send execution packets, approved controlled-send receipts are recorded only when scoped details are supplied under `config.execution`, `revision_requested` decisions create revised packets plus fresh pending owner approval, and `rejected` decisions create closed no-send stop packets |
...
| `payment_link.prepare` | Human approval | Registered command prepares owner-reviewable blocked payment-link packets through the Core worker-run lifecycle; live provider creation and money movement remain blocked |

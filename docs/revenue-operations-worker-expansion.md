...
| Workflow worker command execution | Queued `worker_command` steps can invoke registered worker commands through `/workflow` execution; command name, worker role/id, idempotency key, and command config live in the step input, tenant scope is derived from the claimed workflow tenant, and outputs are recorded back to the step, run, and task |

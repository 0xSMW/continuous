...
| Idempotency | Core `worker.run.start` / `worker.run.complete` owns first-class run lifecycle state for read/classify/draft/run/quote/payment-link paths, with event idempotency kept as a compatibility guard |

...
| `payroll_input.prepare` | `worker.workforce.payroll_input.prepare` | `employmentId`, `period` | Required | Payroll input packet only | Dry-run |
...
| `payroll_preview.prepare` | 2 | Worker | Payroll input preview only | Required | Dry-run |
...
proof, compensation approval, classification rationale, payroll blockers, and
adapter receipts. Tax identifiers, government IDs, health information, bank

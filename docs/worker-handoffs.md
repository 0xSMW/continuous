...
| Command config | Minimal `config.sourceRefs`, `config.policy`, or packet selector the consumer may accept under the generic `/worker` payload |
...
| Handoff | Producer | Consumer | Trigger | Required Core refs | Command config | Acceptance checks | External execution posture |
|---|---|---|---|---|---|---|---|
...
| `revenue.quote_to_pricing` | Revenue Operations | Offer and Pricing | Revenue quote draft needs margin, discount, or change-order policy review | `quoteObjectId`, `leadObjectId`, `customerObjectId`, `evidencePacketId`, `approvalRequestId`, `workflowRunId` | `config.sourceRefs.quoteObjectId`, `config.sourceRefs.evidencePacketId`, `config.policy.marginRuleId`, `config.policy.discountPolicyId`, `config.policy.requireOwnerApproval` | Quote has line items, service area, policy refs, and no approved external send | pricing review only |
| `customer.signal_to_experience` | Revenue Operations or Dispatch/Ops | Customer Experience | Feedback, complaint, testimonial, review, or promise record is ingested | `customerSignalObjectId`, `customerObjectId`, `eventId`, `evidenceId`, optional `jobObjectId` | `config.sourceRefs.customerObjectId`, `config.sourceRefs.customerSignalObjectId`, `config.sourceRefs.evidencePacketId`, `config.policy.requiresOwnerApproval`, `config.policy.allowExternalSend` | Signal type, source, severity/topic, and customer ref are present; outbound recovery is blocked | customer recovery draft only |
| `customer.signal_to_growth` | Customer Experience, Revenue Operations, or Dispatch/Ops | Growth | Review, testimonial, referral opportunity, campaign idea, or customer signal is ready for campaign drafting | `customerSignalObjectId` or `customerSignalId`, `customerObjectId`, `reviewObjectId`, `evidencePacketId` | `config.sourceRefs.customerSignalObjectId` or `config.sourceRefs.customerSignalId`, `config.sourceRefs.evidencePacketId`, `config.policy.channel`, `config.policy.audience`, `config.policy.allowPublish` | Signal or review source exists, customer ref is tenant-scoped, source-backed claims exist or are explicitly missing, Core budget settlement is available, and external publish is false | campaign draft only |
| `dispatch.asset_need_to_supply` | Dispatch/Ops | Asset and Supply | Closeout, exception, schedule, or work-order evidence names a material, asset, or vendor need | `jobObjectId`, `workOrderObjectId`, `assetObjectId`, `materialObjectId`, `evidencePacketId`, optional `cashPacketId` | `config.sourceRefs.workOrderObjectId`, `config.sourceRefs.evidencePacketId`, one of `config.sourceRefs.assetObjectId` or `config.sourceRefs.materialObjectId`, `config.policy.requiresOwnerApproval`, `config.policy.allowPurchase` | Need is tied to a job/work order, purchase action is unapproved, and cash impact is visible or explicitly missing | purchase/maintenance draft only |
| `growth.campaign_to_owner_review` | Growth | Owner Chief-of-Staff | Campaign, content, channel, audience, or attribution packet is ready for publish review | `campaignObjectId`, `contentDraftObjectId`, Core budget settlement refs, `evidencePacketId`, `approvalRequestId` | `config.packet.approvalRequestId` plus `config.sourceRefs.campaignObjectId` | Claims have source refs, Core budget settlement is recorded, audience/channel are explicit, and external publish is false | read-only review |
| `systems.connection_to_packaged_worker` | Systems | Vertical packaged workers | Connector health, permission review, or data-quality repair unblocks a packaged worker flow | `connectionId`, `permissionGrantId`, `dataQualityIssueObjectId`, `evidenceId`, `rollbackPlanDocumentId` | `config.packageKey`, `config.sourceRefs.connectionId`, `config.sourceRefs.permissionGrantId`, `config.policy.allowExternalExecution`, `config.policy.requireRollbackProof` | Tenant scope, least-privilege grant, freshness, and rollback evidence pass before the packaged worker reads the connection | read-only unlock |
...
| `core.connection_to_systems_review` | `core.connection.health.recorded` or scheduler/audit finding with stale, failed, or overbroad connector state | `worker.role=systems_operations`, `command=connector.health.scan` or `command=sync.repair.plan` | `handoff.connection_missing`, `handoff.credential_ref_secret`, `handoff.issue_state_missing`, `handoff.external_repair_requested` | Systems health view, dry-run repair plan, rollback document, and blocked permission/automation tasks |
| `customer.signal_to_experience` | `customer.signal.recorded`, signal `open`, customer and evidence refs present | `worker.role=customer_experience_operations`, `command=recovery.draft` | `handoff.customer_signal_missing`, `handoff.customer_ref_missing`, `handoff.signal_source_missing`, `handoff.external_send_requested` | Recovery draft packet, escalation task, owner approval request, generated `signals` view, and no-send proof |
| `dispatch.asset_need_to_supply` | `dispatch.closeout.prepared` or `dispatch.exception.routed`, material/asset need present | `worker.role=asset_supply_operations`, `command=reorder.plan` | `handoff.work_order_missing`, `handoff.asset_or_material_missing`, `handoff.cash_posture_missing`, `handoff.purchase_already_allowed` | Reorder or maintenance packet, vendor/cash impact review, approval request, rollback plan, and no-purchase proof |
| `customer.signal_to_growth` | `customer.signal.recorded`, review/testimonial/referral opportunity present, source evidence available | `worker.role=growth_operations`, `command=campaign.draft` | `handoff.customer_signal_missing`, `handoff.source_evidence_missing`, `handoff.claim_source_missing`, `handoff.external_publish_requested` | Campaign draft packet with source-backed claims, audience/channel plan, owner approval request, generated `campaigns` view, Core budget settlement, and no-publish proof |
...
| `systems.connection_to_packaged_worker` | `systems.connector.health.scanned`, connection active/fresh, least-privilege grant present | `worker.role=vertical_packages`, `command=package.flow.prepare` | `handoff.connection_stale`, `handoff.permission_grant_missing`, `handoff.grant_overbroad`, `handoff.rollback_missing` | Package readiness packet, family flow plan, generated `package_readiness` view, and no-execution proof |
...
from `config`, starts/completes through the Core worker-run lifecycle, prepares
a source-backed campaign draft packet and `campaigns` view, and blocks publish,
send, spend, and tracking mutation.

...
| Added controlled execution config | Revenue approval continuation still uses `/worker` with `command=continue`; approved controlled-send receipt details live under `config.execution`, include scoped connection/credential/receipt/rollback proof, and are bound into the idempotency hash |

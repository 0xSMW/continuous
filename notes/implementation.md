...
| Hardened worker POST body handling | `POST /worker` now rejects invalid bearer credentials before reading the request body, bounds command bodies at 1 MiB, and records authenticated malformed-body attempts before returning body-shape errors |
| Required managed worker credential inventory | `/worker` read and write routes now require a persisted managed credential row with a token fingerprint after catalog auth succeeds, so missing inventory, unknown tenants, and fingerprintless rows fail closed before registry dispatch |

...
| Readiness boundary | The production readiness gate is strict and opt-in; it is expected to fail until object-storage credentials, backup and observability timers, alert webhook, recovery drill report, real token-rotation/auth-audit/credential-inventory/revocation/session-review references, production connector credentials, and non-root host access are all actually provisioned and attested |

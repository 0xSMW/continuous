...
| Added worker continuation command | `POST /worker` `command=continue` is a generic idempotent continuation surface; V1 consumes `config.approvalId` for `revision_requested` approvals, prepares a revised no-send packet, creates document/evidence packet records plus a fresh pending approval, and keeps external execution blocked |

...
| Decision | Rationale |
|---|---|
| Moved Workforce command runs onto Core lifecycle | `hire.packet.prepare` and `payroll_input.prepare` still enter through the generic `/worker` envelope, but now start and complete through Core `worker.run.start` and `worker.run.complete`, settle budget through the Core run row, attach Workforce business output to the Core run, and keep blocked/dry-run policy fields from being overridden by caller config |
| Moved Dispatch schedule proposals onto Core lifecycle | Dispatch `schedule.propose` still enters through `/worker` or `continuous.worker.command` with source refs, constraints, and policy under `config`, but now starts and completes through Core `worker.run.start` and `worker.run.complete`, settles budget through Core, stores the schedule packet output on the Core run row, and avoids duplicate local `continuous.worker` run rows |
| Moved Dispatch customer updates onto Core lifecycle | Dispatch `customer_update.draft` still enters through the generic `/worker` envelope with job refs, update kind, channel, and message context under `config`, but now starts and completes through Core `worker.run.start` and `worker.run.complete`, stores the blocked draft packet output on the Core run row, settles budget through Core, and avoids duplicate local `continuous.worker` run rows |
| Moved Dispatch closeouts onto Core lifecycle | Dispatch `closeout.prepare` still enters through `/worker` with work-order refs, QA checklist, billable lines, and Finance handoff policy under `config`, but now starts and completes through Core `worker.run.start` and `worker.run.complete`, stores the closeout packet output on the Core run row, settles budget through Core, and avoids duplicate local `continuous.worker` run rows |
| Moved Dispatch exception routing onto Core lifecycle | Dispatch `exception.route` still enters through `/worker` with job refs, reason, severity, and source evidence under `config`, but now starts and completes through Core `worker.run.start` and `worker.run.complete`, stores the blocked exception decision output on the Core run row, settles budget through Core, and avoids duplicate local `continuous.worker` run rows |
| Fixed backup readability for non-root readiness | Postgres backup timer installs now persist `READINESS_USER`, existing backup dumps are group-readable for the readiness user, and future backup dumps/checksum sidecars are written `0640` for that group so strict non-root observability can verify local checksums |
| Moved Compliance filing prep onto Core lifecycle | Compliance `filing.prepare` still uses `/worker` and `continuous.worker.command` with filing inputs under `config`, but now starts and completes through Core `worker.run.start` and `worker.run.complete`, stores the filing packet proof on the Core run row, settles budget through Core, and avoids duplicate local `continuous.worker` run rows |
| Moved Systems command runs onto Core lifecycle | Systems commands still use the generic `/worker` and app-server envelopes, but now start through Core `worker.run.start`, attach connector/permission/repair/automation proof to the Core run row, complete through Core `worker.run.complete`, settle budget through the shared usage ledger, and avoid duplicate local `continuous.worker` run rows |
| Published canonical worker API shape metadata | `continuous.worker.schema`, the worker tool schema, and the app-server worker manifest now expose `apiShape` so future workers and clients learn `/worker`, strict command/view envelope fields, worker selector fields, app-server tool names, and `config` paths from machine-readable metadata instead of first-worker prose |
| Tightened promotion template payloads | Worker expansion templates now keep handoff names in promotion metadata, not `config.sourceRefs`, and template configs validate against the command schemas so future workers do not learn stale or non-executable payload shapes |
...
| Moved Finance invoice prep onto Core lifecycle | Finance `invoice.prepare` still enters through the generic `/worker` envelope, but now uses Core `worker.run.start` and `worker.run.complete` for the worker-run row, budget reservation, usage settlement, and replay repair while Finance keeps invoice, approval, adapter dry-run, evidence, workflow, and view proof |
| Moved Finance follow-up, forecast, and payment drafts onto Core lifecycle | Finance `ar_followup.draft`, `cash_forecast.generate`, and `payment_draft.prepare` still enter through the generic `/worker` envelope with command-specific `config`, but now use Core `worker.run.start` and `worker.run.complete` for the run row, budget reservation, usage settlement, and replay repair while Finance keeps AR, cash forecast, payment, approval, evidence, workflow, and view proof |
...
| Added generic Core ledger reads | `POST /core` and `continuous.core.view` now expose `view=ledger` with `config.collections` and `config.limit`, keeping headless reads on the generic Core envelope instead of adding worker-family or table-specific routes |
| Moved Owner follow-up commands onto Core lifecycle | Owner `decision_queue.prepare` and `anomaly.triage` still enter through the generic `/worker` command envelope, but now start and complete through Core `worker.run.start` and `worker.run.complete`, use explicit Owner capabilities, settle budget through the Core run row, and leave Owner business events/audit/evidence as role-qualified proof |
| Required explicit production catalog scopes | Production control-plane catalog credentials now require `allowedTenants` and `allowedWorkerRoles`, with `"*"` as the visible wildcard, so omitted lists cannot silently broaden tenant or worker-role access |
...
| Moved send receipt validation into Core | `config.execution` remains nested under the generic `/worker` continuation payload, but the approved `customer_message.send` receipt is now validated by Core adapter execution code with tenant-scoped approval proof, managed credential refs, write scope, receipt, rollback, and redacted output |
| Recorded sends as Core external actions | Approved `customer_message.send` continuations now also write a Core `external_action.record` receipt against the tenant-scoped `adapter_action`, keeping the send ledger reusable outside Revenue without adding a send-specific route |
...
| Required explicit control-plane config and worker idempotency | `POST /core`, `/workflow`, and `/approval` command/read payloads now require object `config`, and local plus app-server worker commands require `idempotencyKey`, keeping every control envelope explicit before dispatch |
...
| Added authenticated app-server bridge | `POST /app-server` now accepts only dynamic tool-call fields at the top level, authenticates through the `app_server` control-plane route with exact bridge command scope, builds transport context server-side, and keeps operation inputs under `arguments.config` |
...
| Guarded app-server context CLI plumbing | `bun run app-server:tools` accepts only trusted-local context through `--context` or `APP_SERVER_TRANSPORT_CONTEXT_JSON` and rejects forged `source: "control_plane"` context; real control-plane context must come from an authenticated bridge |
...
| Added app-server workflow and approval tools | `POST /app-server` now exposes `continuous.workflow.schema/view/command` and `continuous.approval.schema/view/command`, preserving the generic `/workflow` and `/approval` envelopes while keeping route-qualified `app_server:*` auth and tenant scope at the bridge boundary |
| Moved deploy control-plane attestation before smoke | Tenant-scoped app-server workflow and approval smoke now runs only after deploy refreshes the managed control-plane credential inventory, so new `app_server:*` command scopes are present before bridge calls require managed credentials |
...
| Adopted Core worker-run lifecycle for split Revenue actions | Revenue `lead.classify` and `response.draft` now use Core `worker.run.start` and `worker.run.complete` for the worker-run row, capability proof, budget reservation, and usage settlement while keeping role-qualified Revenue inference, evidence, audit, and business event proof |
...
| Hardened alert readiness proof | Strict production readiness now POSTs a compact `readiness_probe` payload to an HTTPS `ALERT_WEBHOOK_URL` and records `alert_webhook_reachable`, while observability alerts include a `text` field, so an empty, non-HTTPS, or unreachable alert destination cannot satisfy the customer-data gate |
| Installed bootstrap alerting | Production observability now has a generated external alert webhook stored only in `/etc/continuous/observability.env`; the non-root strict readiness run passed with backup, object-storage, recovery-drill, token, credential, auth-session, non-root, TLS, strict observability, and alerting evidence |
...
| Added payment-link approval continuations | Revenue `command=continue` now accepts `payment_link_approval` through the Core worker-run lifecycle, writes payment-link-specific approved/revision/rejected packets, reuses `payment_link_approval` for revised owner review, and still blocks provider link creation, external mutation, and money movement |
...
| Delegated private readiness env access | Backup and observability timer installers now accept `READINESS_USER`, and non-root access setup delegates existing private timer env files to that user's group with read-only permissions so strict readiness can verify configured backup and alert settings without a root deploy session |
...
| Worker run lifecycle | Core `worker.run.start` and `worker.run.complete` are now the idempotent lifecycle boundary for Revenue Operations Worker `lead.read`, `lead.classify`, `response.draft`, `run`, `quote.prepare`, `payment_link.prepare`, and `continue`, Owner `brief.generate`, Customer Experience `recovery.draft`, Offer/Pricing `margin.review.prepare`, and Finance `invoice.prepare`, with role-qualified business events kept as the audit log |
...
| Readiness boundary | The production readiness gate is now strict and default-on for manual DigitalOcean deploys; it is expected to fail until observability timer/webhook, a verified recovery drill report, production connector credentials, and non-root deploy use are all actually provisioned and attested |
...
| App-server control boundary | Workflow and approval dynamic tools are bridge wrappers over the shared services, not new business routes; outer auth uses exact `app_server:workflow.*` and `app_server:approval.*` commands, while inner transport context uses `workflow:*` and `approval:*` command scopes |
...
| Reserved route-noun worker roles | The shared worker envelope now rejects reserved control-plane route names such as `worker`, `workers`, `api`, `core`, `workflow`, `approval`, and `app_server` as worker roles, so `/worker` cannot become a disguised route-family API |
| Removed concrete legacy route fixtures | Worker contract tests now use generic fake legacy route names for rejection cases, so examples do not imply a historical Revenue-specific URL while still guarding against family-specific API shapes |
| Added legacy worker URL production smoke | `scripts/smoke-production-on-host.sh` now fails deploy smoke if old worker-family URL aliases behave as live worker endpoints instead of absent routes |

...
- Keep app-server/local worker mutation controls on the same `command`, `worker`, `idempotencyKey`, and `config` envelope as `POST /worker`, and keep app-server/local worker reads on `view`, `worker`, and read filters under `config`; do not add worker-family-specific package scripts, app-server tools, or top-level operator/context fields that bypass the command registry.

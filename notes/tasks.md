...
- Keep local worker mutation controls on the same `command`, `worker`, `idempotencyKey`, and `config` envelope as `POST /worker`, and keep local worker reads on `view`, `worker`, and read filters under `config`; do not add worker-family-specific package scripts, app-server tools, or top-level operator/context fields that bypass the command registry.
...
- Make normal DigitalOcean deploys non-root by default, make the production-readiness gate mandatory for customer-data deploys, and add host bootstrap provisioning for backup, observability, and non-root readiness.
- Add the Revenue completion gate and post-Systems worker sequencing to `docs/worker-roadmap.md`, then extend handoff fixture rows for the post-initial worker families.

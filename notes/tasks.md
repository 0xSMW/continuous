...
- Keep `docs/worker-readiness.md` and `docs/worker-handoffs.md` updated in the same change as any worker promotion or new cross-worker fixture; Dispatch now has schedule proposal, customer update draft, closeout packet, and exception routing proof, Finance now has invoice draft/cash packet proof, and the next launch slices are live credential gating plus Finance AR/cash/payment runtime depth.

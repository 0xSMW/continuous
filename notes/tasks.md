...
- Run `scripts/recovery-drill.sh` against a disposable droplet, then install the observability timer with a real alert webhook, complete token rotation, and add non-root host access before using the production droplet for real customer data.

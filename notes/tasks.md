...
- Add idempotent replay support for `/workflow command=transition`, including stable transition keys and replay responses for retries.
...
- Keep Next MCP for Next.js diagnostics; keep app-server worker tooling registry-backed, trusted-local, and free of production-token plumbing; CI now proves `continuous.worker.command` can execute real Revenue commands, so extend that proof to Dispatch/Owner as their runtime surfaces mature and add a production-grade auth boundary before remote app-server use.

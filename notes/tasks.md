...
- Add success-path and command-dispatch route tests for the remaining `/core` primitives (`task.transition`, `object.upsert`, `object.link`, `event.ingest`, `evidence.attach`, `document.create`, `packet.prepare`, `decision.record`, `approval.request`, `capability.grant`, `budget.reserve`, `budget.charge`, `budget.release`, and `view.publish`) plus successful `GET /core` summary before calling Core route coverage complete.
- Keep Next MCP for Next.js diagnostics; keep app-server worker tooling registry-backed, trusted-local, and free of production-token plumbing; CI now proves `continuous.worker.command` can execute real Revenue, Owner, and Dispatch commands, so extend that proof to Finance payment-depth commands as they mature and add a production-grade auth boundary before remote app-server use.

...
const ids = {
  ...
  workOrderObject: "33333333-3333-4333-8333-000000000013",
  ...
  quoteLineObject: "33333333-3333-4333-8333-000000000017",
  ...
  documentQuote: "88888888-8888-4888-8888-000000000006",
  packetQuote: "88888888-8888-4888-8888-000000000007",
...
async function seed() {
  ...
  await db
    ...
    .values([
      ...
      },
      {
        ...
        autonomyLevel: 2,
        scope: {
          flows: ["promise_to_delivery", "schedule_proposal", "customer_update", "closeout", "dispatch_exception"],
      ...
      },
      {
        ...
        mission:
          "Turn revenue quote handoffs into margin, discount, and price-policy review packets while price publish and customer sends stay blocked.",
        autonomyLevel: 2,
        scope: {
          flows: ["quote_to_pricing", "margin_review", "discount_exception", "price_policy"],
          systems: ["price_book", "quote_lines", "approval_policy"],
        },
        memory: {
          pricing_context: "tenant_scoped",
          cost_basis: "owner_only",
        },
        policy: {
          external_execution: "blocked",
        ...
        kpis: {
          pricing_reviews_prepared: 0,
          margin_exceptions_found: 0,
          discount_exceptions_routed: 0,
  ...
  await db
    ...
    .values([
      ...
      {
        id: capIds.marginReviewPrepare,
        key: "margin.review.prepare",
        name: "Prepare margin review",
        class: "policy",
        risk: "high",
        sideEffect: "internal",
        description:
          "Prepare quote-line margin, discount, and price-policy review packets without external price publish or customer send.",
  ...
  await db
    ...
    .values(
      [
        ...
        capabilityId,
        actorType: "worker" as const,
        actorId: ids.offerPricingWorker,
        scope: {
          tenant_id: ids.tenant,
          objects: ["offer", "price_book", "quote", "quote_line", "margin_rule", "discount_policy"],
        },
        policy: {
          mode: "review_only",
          autonomy_level: 2,
  ...
  await db
    ...
    .values({
      id: ids.budgetPolicy,
      tenantId: ids.tenant,
      key: "worker.revenue_operations.standard",
      target: "worker",
  ...
  await db
    ...
    .values([
      ...
      {
        id: ids.offerPricingBudgetAccount,
        tenantId: ids.tenant,
        policyId: ids.budgetPolicy,
        name: "Offer and Pricing Worker monthly intelligence budget",
        target: "worker",
        targetId: ids.offerPricingWorker,
      },
      {
  ...
  await db
    ...
    .values([
      ...
      {
        id: ids.offerPricingBudgetAllocation,
        tenantId: ids.tenant,
        poolId: ids.budgetPool,
        accountId: ids.offerPricingBudgetAccount,
        units: 1000000,
        startsAt: now,
        endsAt: nextMonth,
      },
  ...
  await db
    ...
    .values([
      ...
      {
        id: ids.priceBookObject,
        tenantId: ids.tenant,
        type: "price_book",
        name: "Standard field-service price book",
        state: "active",
        externalId: "seed-price-book",
        data: {
          currency: "USD",
          version: "2026.05",
          effectiveAt: now.toISOString(),
          serviceAreas: ["roofing"],
          sourcePolicy: "owner_approved_standard_services",
        },
      },
      {
        id: ids.marginRuleObject,
        tenantId: ids.tenant,
        type: "margin_rule",
        name: "Roofing minimum margin",
        state: "active",
        externalId: "seed-margin-rule",
        data: {
          serviceArea: "roofing",
          minMarginPercent: 35,
          targetMarginPercent: 45,
          exceptionPolicy: "owner_approval_required",
          effectiveAt: now.toISOString(),
        },
      },
      {
        id: ids.discountPolicyObject,
        tenantId: ids.tenant,
        type: "discount_policy",
        name: "Owner-approved discount policy",
        state: "active",
        externalId: "seed-discount-policy",
        data: {
          maxDiscountPercent: 10,
          approvalThresholdPercent: 5,
          reasonCodes: ["owner_goodwill", "repeat_customer", "weather_delay"],
          restrictedOffers: [],
        },
      },
      {
        id: ids.quoteLineObject,
        tenantId: ids.tenant,
        type: "quote_line",
        name: "Roof leak inspection line",
        state: "policy_review",
        externalId: "seed-quote-line",
        data: {
          quoteObjectId: ids.quoteObject,
          offerObjectId: ids.offerObject,
          priceBookId: ids.priceBookObject,
          description: "Roof leak inspection",
          quantity: 1,
      ...
      {
        id: ids.workOrderObject,
        tenantId: ids.tenant,
        type: "work_order",
        name: "Roof leak inspection work order",
        state: "closeout_ready",
        externalId: "seed-work-order",
        data: {
          jobObjectId: ids.jobObject,
          customerObjectId: ids.customerObject,
          scope: "Inspect and patch the roof leak before the bucket gets promoted to operations manager.",
          crewRequirements: ["roofing"],
          siteNotes: "Customer contact and address are redacted by default.",
          riskFlags: [],
        },
      },
      {
  ...
  await db
    ...
    .values([
      ...
      },
      {
        ...
        transitions: {
          draft: ["scoped", "retired"],
          scoped: ["simulating", "retired"],
          simulating: ["approved", "retired"],
      ...
      {
        id: ids.workflowPricingMarginReview,
        key: "pricing_margin_review",
        name: "Pricing margin review",
        purpose: "Review quote-line margin, discount policy, and price-book evidence before any price publish or customer send.",
        domain: "offer_pricing",
        states: {
          order: ["draft", "source_review", "policy_check", "approval_pending", "review_ready", "blocked"],
        },
        transitions: {
          draft: ["source_review"],
          source_review: ["policy_check", "blocked"],
          policy_check: ["approval_pending", "review_ready", "blocked"],
          approval_pending: ["review_ready", "blocked"],
        },
        objects: { required: ["quote", "quote_line", "price_book", "margin_rule", "discount_policy"] },
        approvals: { required: ["pricing_margin_review"], states: { approval_pending: ["pricing_margin_review"] } },
        evidence: { packet: "pricing_review_packet" },
  ...
  await db
    ...
    .values([
      ...
      {
        id: ids.documentQuote,
        tenantId: ids.tenant,
        objectId: ids.quoteObject,
        kind: "quote_approval_packet",
        name: "Roof leak quote approval packet",
        state: "review_ready",
        sensitivity: "medium",
        hash: "bootstrap-quote-approval-packet",
        data: {
          quoteObjectId: ids.quoteObject,
          quoteLineObjectIds: [ids.quoteLineObject],
          offerObjectId: ids.offerObject,
          priceBookId: ids.priceBookObject,
          marginRuleId: ids.marginRuleObject,
          discountPolicyId: ids.discountPolicyObject,
  ...
  await db
    ...
    .values([
      ...
      { tenantId: ids.tenant, fromId: ids.quoteObject, toId: ids.customerObject, type: "for_customer" },
      { tenantId: ids.tenant, fromId: ids.quoteObject, toId: ids.offerObject, type: "quotes_offer" },
      { tenantId: ids.tenant, fromId: ids.quoteLineObject, toId: ids.quoteObject, type: "belongs_to_quote" },
      { tenantId: ids.tenant, fromId: ids.quoteLineObject, toId: ids.priceBookObject, type: "uses_price_book" },
      { tenantId: ids.tenant, fromId: ids.quoteLineObject, toId: ids.marginRuleObject, type: "checked_by_margin_rule" },
      { tenantId: ids.tenant, fromId: ids.quoteLineObject, toId: ids.discountPolicyObject, type: "checked_by_discount_policy" },
      { tenantId: ids.tenant, fromId: ids.offerObject, toId: ids.priceBookObject, type: "priced_by" },
      { tenantId: ids.tenant, fromId: ids.jobObject, toId: ids.quoteObject, type: "from_quote" },
      { tenantId: ids.tenant, fromId: ids.jobObject, toId: ids.workOrderObject, type: "has_work_order" },
      { tenantId: ids.tenant, fromId: ids.workOrderObject, toId: ids.jobObject, type: "fulfills_job" },
  ...
  await db
    ...
    .values({
      ...
      idempotencyKey: "seed-worker-run",
      state: "done",
      mode: "simulation",
      data: {
        input: {
  ...
  await db
    ...
    .values({
      ...
      idempotencyKey: "seed-worker-run:adapter_run",
      state: "done",
      attempt: 1,
      maxAttempts: 3,
      reconciliationState: "matched",
      cursor: "seed-worker-run",
      readCount: 1,
      writeCount: 0,
      receipt: {
        mode: "dry_run",
    ...
    .values([
      {
        ...
        eventId: ids.eventQuote,
        capabilityId: capIds.quotePrepare,
        kind: "revenue_quote_packet",
        name: "Revenue quote evidence packet",
        state: "review_ready",
        sensitivity: "medium",
        evidenceIds: { ids: [ids.evidenceQuote, ids.evidenceAdapterReceipt] },
        documentIds: { ids: [] },
        hash: "bootstrap-revenue-quote-evidence-packet",
        data: {
          handoff: "revenue.quote_to_pricing",
          quoteObjectId: ids.quoteObject,
          leadObjectId: ids.leadObject,
          customerObjectId: ids.customerObject,
          quoteLineObjectIds: [ids.quoteLineObject],
          marginRuleId: ids.marginRuleObject,
          discountPolicyId: ids.discountPolicyObject,
      ...
      },
      {
        id: ids.packetQuote,
        tenantId: ids.tenant,
        documentId: ids.documentQuote,
        objectId: ids.quoteObject,
        kind: "quote_approval_packet",
        name: "Roof leak quote approval evidence packet",
        state: "review_ready",
        sensitivity: "medium",
        evidenceIds: { ids: [ids.evidenceQuote, ids.evidenceAdapterReceipt] },
        documentIds: { ids: [ids.documentQuote] },
        hash: "bootstrap-quote-approval-evidence-packet",
        data: {
          quoteObjectId: ids.quoteObject,
          leadObjectId: ids.leadObject,
          customerObjectId: ids.customerObject,
  ...
  await db
    ...
    .values({
      ...
      actorType: "worker",
      actorId: ids.worker,
      promptHash: "seed-worker-inference",
      request: {
        route: "low_cost_fast",

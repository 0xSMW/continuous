...
export async function draftFinanceArFollowup(input: {
    ...
    if (replay.state === "running") {
      await settleFinanceCoreWorkerRun({
        ...
        evidence: {
          ...
          draftEvidenceId: optionalString(replayOutput.draftEvidenceId) ?? null,
          ...
          inferenceId: optionalString(replayOutput.inferenceId) ?? null,
          externalExecution: "blocked",
          externalMutation: false,
          externalSend: false,
          paymentLink: "blocked",
  ...
  const settledOutput = await settleFinanceCoreWorkerRun({
    ...
    evidence: {
      ...
      approvalRequestId: result.approvalRequestId,
      workflowRunId: result.workflowRunId,
      inferenceId: optionalString(result.output.inferenceId) ?? null,
      externalExecution: "blocked",
      externalMutation: false,
      externalSend: false,
      paymentLink: "blocked",
    ...
    if (replay.state === "running") {
      await settleFinanceCoreWorkerRun({
        ...
        evidence: {
          ...
          eventId: optionalString(replayOutput.eventId) ?? null,
          evidenceId: optionalString(replayOutput.evidenceId) ?? null,
          forecastEvidenceId: optionalString(replayOutput.forecastEvidenceId) ?? null,
          packetId: optionalString(replayOutput.packetId) ?? null,
          documentId: optionalString(replayOutput.documentId) ?? null,
          approvalRequestId: optionalString(replayOutput.approvalRequestId) ?? null,
          workflowRunId: optionalString(replayOutput.workflowRunId) ?? null,

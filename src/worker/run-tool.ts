...
import { executeWorkerTool, workerToolSchema } from "./tools";
...
async function main() {
  ...
  if (!name || name === "schema" || name === "--schema") {
    console.log(JSON.stringify(workerToolSchema, null, 2));
    return;
  }
  ...
  const result = await executeWorkerTool(name, payload);
